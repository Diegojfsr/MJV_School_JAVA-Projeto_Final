...
import mjv.spring.rh.dominio.dtos.PessoaListaDTO;

@Repository
public interface PessoaRepositorio extends JpaRepository<Pessoa, Long> {
	
	@Query("select new mjv.spring.rh.dominio.dtos.PessoaListaDTO(p.id, p.nome, p.telefone, p.email, d.nome)"
			+ "from Pessoa p left join p.departamento d")
	List<PessoaListaDTO> findAllPessoaLista();
	
	@Query(value = "select new mjv.spring.rh.dominio.dtos.PessoaListaDTO(p.id, p.nome, p.telefone, p.email, d.nome)"
